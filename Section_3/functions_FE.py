...
def teff_NH_Ibar(u,lambda_m,mu):
	...
	# 2.10 
	W = C10abq* (I1bar - tr(Identity(len(u)))) + 1/D1abq* (J-1)**2
	return diff(W,F)
# 
...
def create_geometry(dimensions,discretization):
	...
	Inputs:
	- dimensions: list containing [Width, Height]
	- discretization: lists containing [nx, ny]
	...
	## Mesh generation
	mesh  = create_rectangle(MPI.COMM_WORLD, numpy.array([[0,0],dimensions]), discretization, cell_type=CellType.quadrilateral)
	...
	boundaries = [(1, lambda x: numpy.isclose(x[1], 0)),
	              ...
	              (3, lambda x: numpy.isclose(x[1], dimensions[1])),
...
def FE_solving(dimensions, discretization, material, constitutive_law, Time_param, load_magnitude, solver_options,XDMFNAME, filename, plot_or_not):
	...
	points = numpy.array([[dimensions[0]/2, 0., 0.], [dimensions[0]/2, dimensions[1], 0.]])
	...
	pressure_y_0          = numpy.zeros(num_steps+1, dtype=ScalarType)
	displacement_y_Height = numpy.zeros(num_steps+1, dtype=ScalarType)
	# store initial values
	__uinit, __pinit = Xn.split()
	evaluate_point(mesh, __pinit, cells_pressure, points[0], pressure_y_0, 0)
	evaluate_point(mesh, __uinit.sub(1), cells_displacement, points[1], displacement_y_Height, 0)
	# 
	###################################################
	################ Processing #######################
	###################################################
	...
	for n in range(num_steps):
		...
		evaluate_point(mesh, __p, cells_pressure, points[0], pressure_y_0, n+1)
		evaluate_point(mesh, __u.sub(1), cells_displacement, points[1], displacement_y_Height, n+1)
		# Solve tracking
		if plot_or_not:   
			if MPI.COMM_WORLD.rank == 0:
				print(f"Time step {n}, time {t}, dt: {dt.value}, Number of iterations {num_its}, Load {T.value}")  
				print(f"Time {t}, dt: {dt.value}, p {pressure_y_0[n+1]}, u_y {displacement_y_Height[n+1]}")  
	# xdmf.close()
	if MPI.COMM_WORLD.rank == 0:
		import csv
		###################################################
		################ Post-Processing ##################
		###################################################
		print("Last Pressure at 0", pressure_y_0[-1])
		print("Last displacement at H",displacement_y_Height[-1])
		rows = pressure_y_0, displacement_y_Height, timelist
...
def plot_all():
	...
	ax1.plot(rows_float2[2],rows_float2[1],linestyle=':',linewidth=2,label=r'$\tilde{W}_1(I_1,J)+U_2(J)$',color='navy')
...
def plot_hyper_el():
	...
	# 
	fig1, ax1 = plt.subplots()
	ax1.plot(rows_float[2],rows_float[1],linestyle='--',linewidth=2,label='NH ($U_1(J)$)',color='cornflowerblue')
	ax1.plot(rows_float2[2],rows_float2[1],linestyle=':',linewidth=2,label='NH ($U_2(J)$)',color='navy')
	ax1.plot(timeNHd,dispNH,linestyle='None',marker='s',markeredgewidth=0.5,label='NH [28]',color='blue')
	ax1.grid(color='lightgray', linestyle=':', linewidth=0.8)
	ax1.set_xlabel('time (s)')
	ax1.set_ylabel('Displacement (m)')
	ax1.ticklabel_format(axis="y", style="sci", scilimits=(0,0))
	...
	fig1.savefig('Figure_4a.jpg')
	fig2, ax2 = plt.subplots()
	ax2.plot(rows_float[2],rows_float[0],linestyle='--',linewidth=2,label='NH ($U_1(J)$)',color='cornflowerblue')
	ax2.plot(rows_float2[2],rows_float2[0],linestyle=':',linewidth=2,label='NH ($U_2(J)$)',color='navy')
	ax2.plot(timeNHp,pressNH,linestyle='None',marker='s',markeredgewidth=0.5,label='Neo-Hooke [28]',color='blue')
	ax2.grid(color='lightgray', linestyle=':', linewidth=0.8)
	ax2.set_xlabel('time (s)')
	ax2.set_ylabel('Pressure (Pa)')
	ax2.ticklabel_format(axis="y", style="sci", scilimits=(0,0))
