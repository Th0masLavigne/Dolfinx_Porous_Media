...
material = 600000, 0.3, 4e-14, 1e-3, 0.2, 1e10, 2.2e9
# 
###################################################
#################### FEM-processing ###############
###################################################
...
# Set time counter
begin_t = time.time()
pressure_LE, displacement_LE, timelist_LE = FE_solving([Length,Width,Height],[nx,ny, nz], material, teff_Elastic, time_param, pinit, solver_options,'Elastic','LinearElastic.csv',False)
# Evaluate final time
end_t = time.time()
...
# Set time counter
begin_t = time.time()
pressure_Ibar, displacement_Ibar, timelist_Ibar =FE_solving([Length,Width,Height],[nx,ny, nz], material, teff_NH_Ibar, time_param, pinit, solver_options,'NH_Ibar','NH_Ibar.csv',False)
# Evaluate final time
end_t = time.time()
t_hours = int((end_t-begin_t)//3600)
tmin = int(((end_t-begin_t)%3600)//60)
...
# Set time counter
begin_t = time.time()
pressure_Ibar2, displacement_Ibar2, timelist_Ibar2 =FE_solving([Length,Width,Height],[nx,ny, nz], material, teff_NH_Ibar_2, time_param, pinit, solver_options,'NH_Ibar_2','NH_Ibar_2.csv',False)
# Evaluate final time
end_t = time.time()
t_hours = int((end_t-begin_t)//3600)
tmin = int(((end_t-begin_t)%3600)//60)
...
# Set time counter
begin_t = time.time()
pressure_UJ1, displacement_UJ1, timelist_UJ1 =FE_solving([Length,Width,Height],[nx,ny, nz], material, teff_NH_UJ1, time_param, pinit, solver_options,'NH_UJ1','NH_UJ1.csv',False)
# Evaluate final time
end_t = time.time()
t_hours = int((end_t-begin_t)//3600)
tmin = int(((end_t-begin_t)%3600)//60)
...
# Set time counter
begin_t = time.time()
pressure_UJ2, displacement_UJ2, timelist_UJ2 =FE_solving([Length,Width,Height],[nx,ny, nz], material, teff_NH_UJ2, time_param, pinit, solver_options,'NH_UJ2','NH_UJ2.csv',False)
# Evaluate final time
end_t = time.time()
t_hours = int((end_t-begin_t)//3600)
tmin = int(((end_t-begin_t)%3600)//60)
